...
start_backend_dev() {
  ...
  if [ -z "$api_key" ]; then
    warn "OPENAI_API_KEY 未设置，使用占位值启动（AI 接口可能不可用）"
  ...
  nohup bash -lc "cd \"$BACKEND_DIR\" && OPENAI_API_KEY=\"$api_key\" SPRING_APPLICATION_JSON='$spring_application_json' mvn -DskipTests spring-boot:run" >"$LOG_DIR/backend.log" 2>&1 &
